...
fn is_text_entry(name: &str, bytes: &[u8]) -> bool {
    let text_exts = [
        ".md", ".txt", ".rst", ".csv", ".tsv", ".log", ".yml", ".yaml", ".toml", ".json", ".xml",
        ".ini", ".cfg", ".rs", ".py", ".js", ".ts", ".go", ".rb", ".lua", ".sh", ".bash", ".zsh",
        ".c", ".cpp", ".h", ".hpp", ".java", ".html", ".htm", ".css", ".scss", ".sql", ".graphql",
    ];
...
impl ZipAdapter {
    ...
    pub fn rewrite_zip(path: &Path, replacements: &[(String, String)]) -> Result<(), String> {
        ...
        let tmp_file = fs::File::create(&tmp_path).map_err(|e| format!("create tmp zip: {}", e))?;
        ...
        fs::rename(&tmp_path, path).map_err(|e| format!("rename zip: {}", e))?;
        Ok(())
    }
}
